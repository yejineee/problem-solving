...
int main(){
  ...
  sort(loc_boundR.begin(), loc_boundR.end()) ; 
  ...
  // for(int i = 0 ; i < N ; i++){
  //   printf("(x - %.0lf)^2 + (y - %.3lf)^2 = %.3lf^2\n",loc_boundR[i].first, radius[i], radius[i]) ;
  // }
  printf("%d\n", N) ; 
  for(int i = 0 ; i < N ; i++){
    printf("%lf %lf\n", loc_boundR[i].first, loc_boundR[i].second) ; 
  }
  // for(int i = 1 ; i < N ; i++){
  //   printf("----------------%.0lf ( ~ %.3lf)-------------\n", loc_boundR[i].first, loc_boundR[i].second) ; 
  //   for(int j = i-1 ; j >= 0 ; j--){
  //     printf("%.0lf ) max : %.3lf\n", loc_boundR[j].first, get_max_r(loc_boundR[j].first, radius[j], loc_boundR[i].first)) ; 
  //   }
  //   printf("radius ; %.3lf\n",radius[i]) ;
  // }
  // for(int i = 1 ; i < N ; i++){
  //   printf("----------------%.0lf ( ~ %.3lf)-------------\n", loc_boundR[i].first, loc_boundR[i].second) ; 
  //   for(int j = i-1 ; j >= 0 ; j--){
  //     printf("%.3lf\n", get_max_r(loc_boundR[j].first, radius[j], loc_boundR[i].first)) ; 
  //   }
  //   printf("radius ; %.3lf\n",radius[i]) ;
  // }
}